...
class RandCrop3D(CenterCrop):
    def sample(self, *shape):
        ...
        size = list(self.size) if not isinstance(self.size, list) else self.size

        # Amount of padding needed on each dim (H, W, D)
        self.pad_needed = [max(0, i - s) for i, s in zip(size, shape)]

        # After padding, the effective shape is:
        eff_shape = [s + p for s, p in zip(shape, self.pad_needed)]

        # Now randint range is always valid
        start = [random.randint(0, s - i) for i, s in zip(size, eff_shape)]

        self.buffer = [slice(None)] + [slice(st, st + k) for st, k in zip(start, size)]
        return size

    def tf(self, img, k=0):
        # Pad if needed (pad only spatial dims; keep batch/channel)
        if any(self.pad_needed):
            # img shapes:
            # x: (N, H, W, D, C)
            # y: (N, H, W, D)
            if img.ndim == 5:
...
class RandCrop3D_Loc(CenterCrop):
    def sample(self, *shape):  # shape : [240,240,155]
        assert len(self.size) == 3  # random crop [H,W,T] from img [240,240,155]
        if not isinstance(self.size, list):
            size = list(self.size)
